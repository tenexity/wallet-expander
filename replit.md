...
- **Core Features:**
    ...
    - **Deployment:** Uses autoscale deployment with a lightweight bootstrap (`server/bootstrap.ts` → `dist/bootstrap.cjs`, ~900B). Bootstrap starts an HTTP server instantly (~700ms), responds 200 to healthchecks immediately, then async-imports and initializes the full Express app (`dist/app.cjs`, 1.6MB). Full app takes over request handling once initialized (~2-3s). Dev mode uses `server/index.ts` directly.
